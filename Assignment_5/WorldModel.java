...
public class WorldModel
{
   ...
   public WorldModel(int numRows, int numCols, Background background)
   {
      ...
      for (int row = 0; row < numRows; row++)
      {
         ...
         {
            Node<Id> empty = new Node<Id>(new Id(new Point(row, col), 1000, 1000, 1000), null);
   ...
   public void fillNodes(Point start)
   {
      for (int row = 0; row < numRows; row++)
      {
         for (int col = 0; col< numCols; col++)
         {
            Node<Id> empty = new Node<Id>(new Id(new Point(row, col), 1000, 0, Astar.hScoreFunc(new Point(row, col), start)), null);
            nodes[row][col] = empty;
         }
      }
